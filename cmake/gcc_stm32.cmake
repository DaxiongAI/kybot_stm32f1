...
IF(NOT TOOLCHAIN_PREFIX)
        ...
        #SET(TOOLCHAIN_PREFIX "/cygdrive/c/cygwin/home/Administrator/convert")
...
if(${USE_WHICH_COMPILER} STREQUAL "win_keil")
        CMAKE_FORCE_C_COMPILER(${TOOLCHAIN_BIN_DIR}/${TARGET_TRIPLET}cc GNU)
        CMAKE_FORCE_CXX_COMPILER(${TOOLCHAIN_BIN_DIR}/${TARGET_TRIPLET}cc GNU)
        SET(CMAKE_ASM_COMPILER ${TOOLCHAIN_BIN_DIR}/${TARGET_TRIPLET}cc)

        #       SET(CMAKE_OBJCOPY ${TOOLCHAIN_BIN_DIR}/${TARGET_TRIPLET}-objcopy CACHE INTERNAL "objcopy tool")
        SET(CMAKE_OBJDUMP ${TOOLCHAIN_BIN_DIR}/native-fromelf CACHE INTERNAL "objdump tool")
else()
...
if(${USE_WHICH_COMPILER} STREQUAL "win_keil")
        SET(CMAKE_C_FLAGS_DEBUG "-g" CACHE INTERNAL "c compiler flags debug")
        SET(CMAKE_CXX_FLAGS_DEBUG "-g" CACHE INTERNAL "cxx compiler flags debug")
...
FUNCTION(STM32_ADD_HEX_BIN_TARGETS TARGET)
    ...
    ENDIF()
    if(${USE_WHICH_COMPILER} STREQUAL "win_keil")
        ADD_CUSTOM_TARGET(${TARGET}.hex COMMAND ${CMAKE_OBJCOPY} -Oihex ${FILENAME} ${FILENAME}.hex)
        ADD_CUSTOM_TARGET(${TARGET}.bin COMMAND ${CMAKE_OBJDUMP} --bin ${FILENAME} --output ${FILENAME}.bin)
    else()
        #ADD_CUSTOM_TARGET(${TARGET}.hex DEPENDS ${FILENAME} COMMAND ${CMAKE_OBJCOPY} -Oihex ${FILENAME} ${FILENAME}.hex)
        #ADD_CUSTOM_TARGET(${TARGET}.bin DEPENDS ${FILENAME} COMMAND ${CMAKE_OBJCOPY} -Obinary ${FILENAME} ${FILENAME}.bin)
        ADD_CUSTOM_TARGET(${TARGET}.hex COMMAND ${CMAKE_OBJCOPY} -Oihex ${FILENAME} ${FILENAME}.hex)
...
INCLUDE(${PROJECT_SOURCE_DIR}/cmake/gcc_stm32f1.cmake)
